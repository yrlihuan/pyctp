       ...
       prefix, prefix))
            ...
            for j in range(len(args['func_args'])):
                ...
                if t=='ZQTHOST_TE_RESUME_TYPE':
                    f.write(u"  ZQTHOST_TE_RESUME_TYPE %s = (ZQTHOST_TE_RESUME_TYPE)PyInt_AsLong(py_%s);\n" % (n,n))
                elif t=='THOST_TE_RESUME_TYPE':
            ...
            for j in args['func_args']:
                if self._arg_type_map(j['type'])=='N':
                    if not 'Spi' in j['name'] and j['type']!='THOST_TE_RESUME_TYPE' and j['type'] != "ZQTHOST_TE_RESUME_TYPE":
                        f.write(u"  free(%s);\n" % j['name'])
...
def test(locale='utf-8'):
    a=Parse("../inc/ThostFtdcMdApiSSE.h", 'Trader',locale)
    from pprint import pprint
    pprint(a.struct)
#test('gbk')

def test2(locale='utf-8'):
    a=Parse("../ctp_sse/api/trade/linux64/public/ThostFtdcTraderApiSSE.h", "Trader",locale)
    a.generate_wrapper()
